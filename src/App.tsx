...
import type { Profile, PlayerCard, HistoricPlayer, Location, AvailabilitySlot, Tournament, Division, Registration, Match, MatchSet, Standings, BookingAdmin, BookingAccount, CourtBookingRequest, SocialEvent, PendingOnboarding, PPCNotif, BookingVenueKey } from './types';
...
const App = () => {
  ...
  // Push notifications
  const { isSupported: pushSupported, permission: pushPermission, isSubscribed: pushSubscribed, subscribe: pushSubscribe, unsubscribe: pushUnsubscribe, isLoading: pushLoading } = usePushNotifications(currentUser?.id ?? null);
  ...
  const fetchData = async (userId?: string) => {
    ...
    try {
      const promises = [
        ...
        (async () => ({ data: await fetchAllRows<Match>('matches'), error: null }))(),
...
//cont.. Copia el Public URL y guárdalo en social_events.image_url.
        ...
        <div className="fixed top-0 left-0 z-50 h-full w-72 max-w-[85vw] flex flex-col bg-gradient-to-b from-green-800 via-emerald-800 to-green-900 shadow-2xl">
          ...
          <div className="px-3 pb-5 pt-2 border-t border-white/10">
            ...
            {pushSupported && pushPermission !== 'denied' && (
              <button
                type="button"
                onClick={() => { if (pushSubscribed) { pushUnsubscribe(); } else { pushSubscribe(); } }}
                disabled={pushLoading}
                className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-left text-sm font-medium text-white/70 hover:bg-white/10 hover:text-white transition-all active:scale-[0.98] disabled:opacity-50 mb-1"
              >
                <span className="w-6 h-6 flex items-center justify-center shrink-0 opacity-80">🔔</span>
                <span>{pushSubscribed ? 'Desactivar notificaciones' : 'Activar notificaciones'}</span>
                {pushSubscribed && <span className="ml-auto text-xs text-emerald-300">●</span>}
              </button>
            )}
            <button
              type="button"
  ...
  async function joinPendingMatch(m: Match) {
    ...
    // Fire-and-forget push notification to the match creator
    const rivalName = profiles.find(p => p.id === uid)?.name || 'Rival';
    const locationName = locations.find(l => l.id === m.location_id)?.name;
    const payload = buildMatchScheduledPayload(
      { date: m.date, time: m.time } as any,
      rivalName,
      locationName
    );
    sendPushNotification(m.home_player_id, payload.title, payload.body, payload.url, payload.actions);
  }
          ...
          {registrationStep === 1 ? (
            <>
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-3 px-4 py-3 border border-gray-300 rounded-lg hover:bg-gray-50 transition duration-200 mb-5"
              >
              ...
              {authTab === 'register' && (
                <form onSubmit={handleRegister}>
                  <div className="space-y-4">
                    ...
                    <p className="text-xs text-gray-500 text-center">Utiliza Nombre + Apellido, y correo personal</p>
                  </div>
                </form>
              )}
            </>
  ...
  if (!selectedTournament) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-green-500 via-emerald-600 to-lime-700">
        ...
        {currentUser && pushSupported && pushPermission === 'default' && !pushSubscribed && !pushBannerDismissed && (
        ...
        {showJoinModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-xl shadow-2xl p-8 w-full max-w-md">
              <h2 className="text-2xl font-bold text-gray-800 mb-2">Unirse a un Torneo</h2>
              <p className="text-sm text-gray-500 mb-6">Selecciona en qué torneo y división participarás.</p>
              <div className="space-y-4">
                {/* Selector de Torneo */}
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Seleccionar Torneo</label>
                ...
                {newRegistration.tournamentId && (() => {
                  const availableDivisions = divisions
                    .filter(d => d.tournament_id === newRegistration.tournamentId)
                    .map(d => {
                      const currentCount = registrations.filter(r =>
                        r.division_id === d.id && r.tournament_id === newRegistration.tournamentId && r.status !== 'retired'
                      ).length;
                      const isFull = d.capacity ? currentCount >= d.capacity : false;
                      return { ...d, currentCount, isFull };
                    });

                  const allFull = availableDivisions.every(d => d.isFull);

                  if (allFull && availableDivisions.length > 0) {
                    return (
                      <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                        <p className="text-sm text-red-800 font-medium">No hay cupos disponibles en este torneo.</p>
                        <p className="text-xs text-red-700 mt-1">
                          Envía tu solicitud al grupo de la PPC o contacta al encargado de la web.
                        </p>
                      </div>
                    );
                  }
                      ...
                      >
                        ...
                        {availableDivisions.map(d => (
                          <option key={d.id} value={d.id} disabled={d.isFull}>
                            {d.name}{d.isFull ? ' (Completa)' : d.capacity ? ` (${d.currentCount}/${d.capacity})` : ''}
                          </option>
                        ))}
                      </select>
                    </div>
                ...
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 mt-2">
                  <p className="text-xs text-amber-800">
                    📋 Recuerda completar tu perfil en <strong>Editar Perfil</strong> (disponibilidad y ubicaciones).
                  </p>
                  <p className="text-xs text-amber-800 mt-1">
                    🎾 Crea tu hoja de perfil en la sección <strong>Ficha</strong> dentro de tu perfil de jugador.
                  </p>
                </div>
                
                {/* Botones de acción */}
                <div className="flex space-x-4 pt-4">
                  {/* Solo mostrar Cancelar si ya tiene al menos un registro (no es usuario nuevo sin torneo) */}
                  {registrations.some(r => r.profile_id === currentUser?.id) ? (
                    <button onClick={() => setShowJoinModal(false)} className="flex-1 bg-gray-500 text-white py-3 rounded-lg font-semibold">Cancelar</button>
                  ) : (
                    <button onClick={handleLogout} className="flex-1 bg-gray-500 text-white py-3 rounded-lg font-semibold text-sm">Cerrar sesión</button>
                  )}
                  <button onClick={handleJoinTournament} className="flex-1 bg-green-600 text-white py-3 rounded-lg font-semibold">Unirse</button>
                </div>
              </div>
            </div>
          </div>

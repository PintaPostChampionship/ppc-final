...
const App = () => {
  ...
  const shareAllScheduledMatches = () => {
    ...
    allScheduled.forEach(match => {
      ...
      message += `${match.date} - ${player1} vs ${player2}\n`;
      message += `L: ${location} | H: ${match.time}\n`;
      message += `\n`;
    });
  ...
  const handleAddMatch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const player1Profile = profiles.find(p => p.name === newMatch.player1);
      const player2Profile = profiles.find(p => p.name === newMatch.player2);
      
      if (!player1Profile || !player2Profile) {
        throw new Error('Player not found');
      }
      
      // Find tournament and division IDs
      const tournament = tournaments.find(t => t.name === newMatch.tournament);
      const division = divisions.find(d => 
        d.tournament_id === tournament?.id && d.name === newMatch.division
      );
      ...
      if (newMatch.location) {
        const location = locations.find(l => l.name === newMatch.location);
      ...
      // Calculate total games won/lost
      ...
      const { data: matchData, error: matchError } = await supabase
        ...
        .insert({
          ...
          status: 'played',
          player1: player1Profile.id,
          player2: player2Profile.id,
          ...
          player1_pints: newMatch.hadPint ? newMatch.pintsCount : 0,
          player2_pints: newMatch.hadPint ? newMatch.pintsCount : 0,
        ...
        if (set.score1 !== '' && set.score2 !== '') {
          const { error: setError } = await supabase
            ...
            .insert({
              ...
              set_number: i + 1,
              p1_games: parseInt(set.score1),
              p2_games: parseInt(set.score2)
            });
  ...
  if (selectedTournament && !selectedDivision) {
    ...
    const divisionsData = tournamentDivisions.map(division => {
      ...
      // Find leader
      const leader = divisionStandings.length > 0 ? 
        divisionStandings.sort((a, b) => b.points - a.points)[0] : null;
      
      // Find player with most pints
      const topPintsPlayer = divisionStandings.length > 0 ? 
        divisionStandings.sort((a, b) => b.pints - a.pints)[0] : null;
      
      // Get leader profile
      const leaderProfile = leader ? profiles.find(p => p.id === leader.profile_id) : null;
      const topPintsProfile = topPintsPlayer ? profiles.find(p => p.id === topPintsPlayer.profile_id) : null;
      ...
      return {
        ...
        winner: leaderProfile ? leaderProfile.name : 'N/A',
        totalPints: divisionStandings.reduce((sum, s) => sum + s.pints, 0),
        leader: leaderProfile,
        topPintsPlayer: topPintsProfile,
        playersList: players
      };
    });
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
              ...
              <form onSubmit={(e) => {
                ...
                const match = {
                  ...
                  date: newMatch.date,
                  time: newMatch.time,
                  location_id: locations.find(l => l.name === newMatch.location)?.id || null,
                  status: newMatch.player2 ? 'scheduled' : 'pending',
                  player1: profiles.find(p => p.name === newMatch.player1)?.id || '',
                  player2: newMatch.player2 ? profiles.find(p => p.name === newMatch.player2)?.id || '' : 'Pending',
                  player1_sets_won: 0,
                  player2_sets_won: 0,
                  player1_games_won: 0,
                  player2_games_won: 0,
